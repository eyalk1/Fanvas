...
class bit_set {
  ...
  static constexpr auto M =
      static_cast<int>(N); // keep size_t from spilling all over the code base
...
public:
  ...
  [[nodiscard]] constexpr bit_set(std::input_iterator auto first,
                                  std::input_iterator auto last) noexcept {
    insert(first, last);
  }

  [[nodiscard]] constexpr bit_set(
  ...
  [[nodiscard]] constexpr auto operator<=>(bit_set const &other
  ...
  [[nodiscard]] constexpr auto begin() const noexcept {
    return const_iterator(this, find_first());
  }
  ...
  [[nodiscard]] constexpr auto full() const noexcept {
    if constexpr (has_unused_bits) {
      if constexpr (num_logical_blocks == 1) {
        ...
        return m_data[0] == used_bits &&
               std::ranges::all_of(m_data | std::views::drop(1),
                                   [](auto block) { return block == ones; });
      }
    } else {
      if constexpr (num_logical_blocks == 0) {
        return true;
        ...
        return std::ranges::all_of(m_data,
                                   [](auto block) { return block == ones; });
      }
    }
  }
  ...
  [[nodiscard]] constexpr auto ssize() const noexcept {
    if constexpr (num_logical_blocks == 0) {
      ...
      return std::accumulate(
          ...
          [](auto sum, auto block) { return sum + std::popcount(block); });
    }
  }
...
private:
  constexpr auto do_insert(value_type x) noexcept -> std::pair<iterator, bool> {
    assert(is_valid(x));
    auto &&[block, mask] = block_mask(x);
...
public:
  constexpr auto insert(value_type const &x) noexcept { return do_insert(x); }
...
public:
  ...
  constexpr auto insert(std::input_iterator auto first,
                        std::input_iterator auto last) noexcept requires
      std::constructible_from<value_type, decltype(*first)> {
    for (auto x : std::ranges::subrange(first, last)) {
      add(x);
    }
  }

  constexpr auto insert_range(std::ranges::range auto &&rg) noexcept requires
      std::constructible_from<value_type, decltype(*rg.begin())> {
    for (auto x : rg) {
      add(x);
    }
  }
  ...
  constexpr auto fill() noexcept {
    if constexpr (has_unused_bits) {
      ...
      if constexpr (num_logical_blocks == 2) {
        ...
        std::ranges::fill_n(std::next(std::begin(m_data)),
                            num_logical_blocks - 1, ones);
      }
    } else {
      if constexpr (num_logical_blocks == 1) {
        m_data[0] = ones;
  ...
  constexpr auto &operator<<=(value_type n [[maybe_unused]]) noexcept {
    ...
    if constexpr (num_logical_blocks == 1) {
      ...
      if (R_shift == 0) {
        std::ranges::copy_n(std::next(std::begin(m_data), n_block),
                            num_logical_blocks - n_block, std::begin(m_data));
      } else {
        ...
        m_data[num_logical_blocks - 1 - n_block] =
            static_cast<block_type>(m_data[num_logical_blocks - 1] >> R_shift);
      }
    ...
    if constexpr (num_logical_blocks == 1) {
      ...
      if (L_shift == 0) {
        std::ranges::copy_backward(
            m_data | std::views::take(num_logical_blocks - n_block),
            std::end(m_data));
      } else {
  ...
  is_subset_of(bit_set const &other [[maybe_unused]]) const noexcept -> bool {
    if constexpr (num_logical_blocks == 0) {
      return true;
    } else if constexpr (num_logical_blocks == 1) {
...
#ifdef __cpp_lib_ranges_zip
      return std::ranges::none_of(std::views::zip(this->m_data, other.m_data),
                                  [](auto const &tup) {
  ...
  [[nodiscard]] constexpr auto
  is_proper_subset_of(bit_set const &other [[maybe_unused]]) const noexcept {
    if constexpr (num_logical_blocks == 0) {
      return false;
    } else if constexpr (num_logical_blocks == 1) {
      return !((this->m_data[0] & ~other.m_data[0]) ||
  ...
  [[nodiscard]] constexpr auto
  intersects(bit_set const &other [[maybe_unused]]) const noexcept -> bool {
    if constexpr (num_logical_blocks == 0) {
      return false;
    } else if constexpr (num_logical_blocks == 1) {
      return this->m_data[0] & other.m_data[0];
...
#ifdef __cpp_lib_ranges_zip
      return std::ranges::any_of(std::views::zip(this->m_data, other.m_data),
                                 [](auto const &tup) {
                                   auto const &[lhs, rhs] = tup;
                                   return lhs & rhs;
                                 });
...
private:
  ...
  static constexpr auto used_bits =
      static_cast<block_type>(ones << num_unused_bits);
  static constexpr auto last_block = num_logical_blocks - 1;
  static constexpr auto last_bit =
      static_cast<block_type>(static_cast<block_type>(1) << (block_size - 1));
  ...
  [[nodiscard]] static constexpr auto div(value_type numer,
                                          value_type denom) noexcept
  ...
  [[nodiscard]] constexpr auto block_mask(value_type n) noexcept
    ...
    return {m_data[last_block - index],
            static_cast<block_type>(last_bit >> offset)};
  }
  ...
  [[nodiscard]] constexpr auto find_front() const noexcept {
    ...
    if constexpr (num_logical_blocks <= 1) {
      ...
      return m_data[1] ? std::countl_zero(m_data[1])
  ...
  [[nodiscard]] constexpr auto find_back() const noexcept {
    ...
    if constexpr (num_logical_blocks <= 1) {
      ...
      return m_data[0] ? num_bits - 1 - std::countr_zero(m_data[0])
  ...
  [[nodiscard]] constexpr auto find_first() const noexcept {
    if constexpr (num_logical_blocks == 1) {
      ...
      for (auto i = num_logical_blocks - 1, n = 0; i >= 0;
           --i, n += block_size) {
        if (auto const block = m_data[i]; block) {
          return n + std::countl_zero(block);
        }
  ...
  [[nodiscard]] constexpr auto find_next(value_type n) const noexcept {
    ...
    if constexpr (num_logical_blocks == 1) {
      ...
      if (offset) {
        if (auto const block = static_cast<block_type>(m_data[i] << offset);
            block) {
          return n + std::countl_zero(block);
        }
        --i;
  ...
  [[nodiscard]] constexpr auto find_prev(value_type n) const noexcept {
    assert(is_valid(n));
    if constexpr (num_logical_blocks <= 1) {
      return n - std::countr_zero(static_cast<block_type>(
                     m_data[0] >> (block_size - 1 - n)));
    } else if constexpr (num_logical_blocks >= 2) {
      auto const [index, offset] = index_offset(n);
      auto i = last_block - index;
      if (auto const reverse_offset = block_size - 1 - offset; reverse_offset) {
        if (auto const block =
                static_cast<block_type>(m_data[i] >> reverse_offset);
            block) {
          return n - std::countr_zero(block);
        }
        ++i;
      ...
      for (/* init-statement before loop */; i < num_storage_blocks - 1;
           ++i, n -= block_size) {
        if (auto const block = m_data[i]; block) {
          return n - std::countr_zero(block);
        }
  ...
  public:
    ...
    [[nodiscard]] explicit constexpr proxy_reference(rimpl_type r,
    ...
    [[nodiscard]] constexpr auto
    operator==(proxy_reference const &other) const noexcept -> bool {
    ...
    [[nodiscard]] explicit(false) constexpr operator T() const
        noexcept(noexcept(T(m_val))) requires std::is_class_v<T> &&
        std::constructible_from<T, value_type> {
      return m_val;
    }
  };
  ...
  public:
    ...
    [[nodiscard]] explicit constexpr proxy_iterator(pimpl_type p,
                                                    value_type v) noexcept
        : m_ptr(p), m_val(v) {
    ...
    [[nodiscard]] constexpr auto
    operator==(proxy_iterator const &other) const noexcept -> bool {
...
template <std::size_t N, std::unsigned_integral Block>
[[nodiscard]] constexpr auto operator&(bit_set<N, Block> const &lhs,
                                       bit_set<N, Block> const &rhs) noexcept {
  auto nrv = lhs;
  nrv &= rhs;
  return nrv;
}
...
template <std::size_t N, std::unsigned_integral Block = std::size_t,
          std::size_t D = std::numeric_limits<Block>::digits>
